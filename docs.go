...
func ParseDocPage(relPath string, data []byte) (*DocPage, error) {
	...
	if err != nil {
		// Frontmatter is optional for docs — treat whole file as body
		body = string(data)
	}

	// Slug is the relative path without .md (e.g. "combat/ai-design")
...
func LoadDocs(dir string) ([]*DocPage, error) {
	...
	sort.Slice(pages, func(i, j int) bool {
		if pages[i].Section != pages[j].Section {
			// Root pages first, then alphabetical sections
			if pages[i].Section == "" {
				return true
			}
			if pages[j].Section == "" {
